...
- **AI Worker (ai-intelligence module, since 2026-08-12)** — the consolidated AI worker (`ai-intelligence/src/worker/aiWorker.ts`) handles long-running LLM tag generation, embeddings, and semantic queries on the shared WebLLM engine, with cancellation support. `useImageStore.startAutoTagging` constructs it via the module's guarded `createAiWorker()` factory (dynamic import, null-safe when the module is absent); the app ships no worker file. Workers are **role-scoped since 2026-09-13**: the app's auto-tag worker is created `{ role: 'chat' }` and the module coordinator's worker `{ role: 'embed' }`, so each engine loads only the record it serves (the other model no longer occupies GPU memory in either worker). Auto-tagging is premium-only — the free built-in fallback was dropped (2026-08-12), so no-module builds get no auto-tagging at all.
- **GPU preference (contract in `services/gpuPreference.ts`, patch in the module)** — one setting for all AI-Intelligence: `aiDevicePreference` (auto / high-performance / low-power / software) steers which GPU Chromium uses for the shared WebLLM engine. WebLLM has no device API, so the bridge's four engine factories call the module's `applyGpuPreference` (ai-intelligence/src/gpu/gpuPreference.ts) before construction — the preference is merged into `navigator.gpu.requestAdapter` options (`powerPreference` for discrete/integrated, `forceFallbackAdapter` for software/SwiftShader). The app keeps only the open-source CONTRACT (`AiDevicePreference` type + `AI_DEVICE_PREFERENCES` const for the settings dropdown); the patch implementation is deliberately closed. The preference is read at message-send time (`useImageStore` 'start' payload, coordinator init/restore/query) and rides every worker payload; the worker reports the adapter it actually requested (`adapter.info`) via a `gpu-info` message → `useImageStore.detectedGpuInfo`, shown as "Detected GPU" in Settings → AI Intelligence. On dual-GPU Windows machines the request-time hint alone can still land on the display-connected adapter, so `electron/main.mjs` also reads the pref at startup and passes the matching Chromium GPU-process switch (`--force_high_performance_gpu` / `--force_low_power_gpu`) before the first window — best-effort (drivers/Windows power plans may override), takes full effect after an app restart.

### Semantic Search (Phases 4–6 — coordinator + store wiring + UI)

- **Coordinator (thin boundary in `services/semanticSearchEngine.ts`, implementation in the module)** — the app file is a ~170-line boundary with the same public surface (positional `(onProgress?, onGpuInfo?)` ctor + `ensureInitialized/indexImages/search/clearIndex/getStatus/dispose`) that lazily loads the module's `SemanticSearchCoordinator` (ai-intelligence/src/coordinator/semanticSearchEngine.ts). The module coordinator owns the AI worker lifecycle, chunked restore of persisted vectors (2,000 records/message), Δ indexing by `textHash` (embed → persist → restore), single-shot `search()` that preempts indexing worker-side (§5.1), `clearIndex()`, and progress reporting. Holds **no vectors** itself — the index lives on the worker heap. The boundary injects `isPremium` (the app's license gate — init rejects without a valid license) and `devicePreference` (Settings → AI Intelligence) callbacks read at send time, and forwards the worker's `gpu-info` message to `setDetectedGpuInfo`.
- **Vector persistence (module: `ai-intelligence/src/storage/semanticVectorsStorage.ts`)** — IndexedDB CRUD over the `semanticVectors` store, hardcoded to the app's `image-metahub-preferences` DB **v8** (the app's `indexedDb.ts` migration creates the store first; the module only opens the existing DB and must never replicate the app's `deleteDatabase` reset — a version bump is a manual coupling check). The renderer touches records only transiently during indexing.
- **Text builder** — the module coordinator uses the module's own `buildSearchableText`/`buildTextHash` internally (never null; empty text is skipped, not errored). The app's premium-gated `createSemanticTextBuilder()` mirror remains exported from `services/aiBridge.ts` (tests only; the Δ detection it used to feed now happens module-side).
...
- Design, phases, and deviation log: `ai-intelligence/docs/semantic-search.md`. UI (Phase 6) is implemented; rerank (Phase 7) is pending.
