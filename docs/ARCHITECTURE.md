...
- **GPU preference (`services/gpuPreference.ts`)** — one setting for all AI-Intelligence: `aiDevicePreference` (auto / high-performance / low-power / software) steers which GPU Chromium uses for the shared WebLLM engine. WebLLM has no device API, so the bridge's four engine factories patch `navigator.gpu.requestAdapter` before construction — the preference is merged into the adapter options (`powerPreference` for discrete/integrated, `forceFallbackAdapter` for software/SwiftShader). It is read at message-send time (`useImageStore` 'start' payload, coordinator init/restore/query) and rides every worker payload; the worker reports the adapter it actually requested (`adapter.info`) via a `gpu-info` message → `useImageStore.detectedGpuInfo`, shown as "Detected GPU" in Settings → AI Intelligence. On dual-GPU Windows machines the request-time hint alone can still land on the display-connected adapter, so `electron/main.mjs` also reads the pref at startup and passes the matching Chromium GPU-process switch (`--force_high_performance_gpu` / `--force_low_power_gpu`) before the first window — best-effort (drivers/Windows power plans may override), takes full effect after an app restart. The ai-intelligence module is untouched.
