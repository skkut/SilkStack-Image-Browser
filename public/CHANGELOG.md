...
- **The compact window grows with the zoom** — magnifying in compact mode left the window exactly the size it opened at, so the picture was cropped by a frame that never moved. The frame now grows into whatever the display still has, and it does so **per axis**: a frame is usually out of room in one direction long before the other, and a portrait file on a wide screen is out of height at its opening size with most of the width still free — so a shared ratio froze exactly where the zoom started. A 1024x1024 that had 3% of headroom now fills the screen as it is zoomed, and a 832x1216 widens across it. The window still never shows background and still never leaves the work area: each axis stops at the display, and the magnification past that point crops and pans inside the frame as before, with zooming back out returning it to the image's own shape.
- **The compact window moves in one motion per zoom step** — the frame was resized and then re-centred as two separate window operations, so each step of a zoom drew the window at its new size in its *old* place before it jumped to where it belonged: 182px off on the way in, 293px on the way out, on every step in both directions. Size and position are now committed together, so there is no in-between position left to see. The window is also asked for its new shape *before* the picture is drawn at the new magnification rather than after it, and a pan that a resized frame no longer allows is corrected on the resize's own frame — three small motions per step, arriving one after another, are now one.
- **Magnifying no longer makes the picture flicker** — every step of a zoom resized the window, and Windows takes 45–95ms to put new content on screen after a resize (measured on a bare window with none of this app in it, so it is not a cost the viewer can render its way out of). Steps of a real flick are ~40ms apart, so at speed the resizes arrived faster than they could be shown and the picture never once appeared at the size it was actually at. That was the flicker. Magnifying now moves only the picture — something the graphics layer does in a single frame — and the window opens around it **when the zoom stops**: a flick's dozen steps are a handful of resizes at the end instead of a dozen through it. A coalesced flick can ask for a frame two and a half times the size it was showing, though, and a resize of that size was the one thing left in this mode that read as a flash — photographed through a zoom run, what is on the glass for the 40–60ms before new content reaches the screen is the previous frame **rescaled** into the new rectangle: the picture at the right size, because the frame follows the magnification, but resampled, with the padding and the drag bar rescaled along with it. That cost is proportional to how far the resize moved, so the growth now lands in steps no larger than a single wheel step's (**1.25**), each one waiting for the last to be shown; ordinary scrolling is untouched, since a wheel step already moves the frame by only 1.11–1.24. Zooming **out** is still applied immediately, so the frame is never left larger than the picture and no background is ever shown; zooming in magnifies inside the frame it has and the frame catches up the moment you stop, the same crop-and-pan the viewer already uses once a window has reached the edge of the screen.
- **The compact window shrinks when you zoom out** — the frame followed the magnification up but never down: once the picture was back at its fit the window stayed the size it had grown to, and scrolling the other way did nothing at all, because the zoom was floored at 1x in three separate places. The floor is now the **window's own minimum** rather than 1x — the frame stops when its dominant axis reaches **40% of the screen**, or when it would be smaller than the window manager will make it, whichever comes first, so a wide panorama stops sooner than a square does. The share is taken on the axis the frame actually fills: a 288px stop on a 2048x1152 display is a quarter of the *height* but only a seventh of the width — 3.5% of the screen's area — which is what "25% of 25%" looked like. At 40% a file shaped like that display stops at **808x454** instead of 505x284, and a square one at **454x454** instead of 288x288. The frame also shrinks **about its centre** rather than holding its corner, so stepping the window down does not walk it towards the top-left of the screen, and a window you have moved by hand keeps the spot you put it in. Zooming out now takes the window down with the picture exactly as zooming in takes it up, and it stops where the frame is still covered by the picture: any smaller and the background this mode exists to hide would show through. There is no way to pace a shrink the way a growth is paced — a frame left larger than its picture *is* the flash — so every step down is a **proportion** (÷1.2) instead of the flat 0.25 the wheel subtracts, which from 1x would have asked for a third of the window in a single commit. The zoom-out button, the slider and Reset all follow the same floor, and Reset is live below the fit, since that is the way back up.
- **The compact window keeps the size you give it** — shrinking the window by scrolling wrote the *magnification*, which belongs to the image you are looking at, so the next image opened at its own fit again and the size you had chosen was gone. The gesture now writes the same remembered size a hand-drag writes, and that number outlives the image: the window you leave is the window the next image opens in. Below the fit it is a **share of the display** rather than a multiple of the image's own fit — the frame's dominant axis comes out at that share of the display dimension the file's fit fills — so it means the same thing for every file: a portrait opened after a landscape lands on the same share of the screen rather than on its own 100%, and two files that run out of the same dimension, a wide landscape and a square on a wide display, open with that edge identical. The hand-drag reader was brought into the same unit: a magnified frame that a hand dragged down to the picture's own size used to be remembered as "one times the fit", a number the sizing rule now reads as a share of the screen and answers with a window several times the size that was dragged. A drag that comes out under the smallest frame the window manager will make is **refused rather than clamped**: the window keeps the size the hand gave it and the remembered size stays at the last one both images could honour, where a clamp would spring the frame back out to a size nobody asked for and a smaller one carried forward would open the next image with its background showing.
- **Wheel-zooming no longer nudges the picture sideways** — every step of a wheel zoom magnified the picture and then moved it to keep the point under the cursor in place, and the window — which grows to hold the magnification — put the picture back in the middle a moment later. Two motions, in opposite directions, for one step: that is what a zooming picture jittering sideways is. The picture is now anchored only on an axis the display has actually **stopped the frame on**, where magnifying really does crop it and the pane it is measured against is not going to change. On an axis the frame can still grow into, the middle is the only place the picture can end up, so that is where it stays: one motion per step, in one direction. A portrait file on a wide screen shows both at once — it runs out of height at its opening size, so the wheel magnifies about the cursor vertically, while the width keeps following the zoom and nothing is pushed sideways.
...
- **Shift+click selects exactly the run you can see** — the run connects the clicked card to the **nearest edge** of the selection: clicking before it fills up to its *first* image, clicking after it extends from its *last*, and a click landing inside fills the gap back to the last image picked. It is always unioned with what is already selected, so nothing is ever dropped, and a selection built part by part keeps its gaps instead of being swept into one block when you click past it. The run is measured in the order the cards are actually drawn — with stacks (or a sorted table) that order is not the library's, so a run could light up images scrolled far away while leaving the cards between the two clicks unselected. Two gestures that used to miss the range entirely now work: a Shift+click on a card's **checkbox** (the box's own handler swallowed the modifier, so it toggled that single image and left the images in between unselected) and a Shift+click continuing a selection built by simply opening images (the run was anchored on the last image *opened* rather than the last one *picked*). Previously a Shift+click could also wipe the selection and pop open the viewer instead.
