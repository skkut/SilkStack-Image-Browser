...
export type ComfyNodeDataType =
  ...
  | 'SIGMAS' | 'NOISE' | 'UPSCALE_MODEL' | 'LATENT_UPSCALE_MODEL' | 'AUDIO' | 'VIDEO'
  | 'MASK' | 'ANY' | 'LORA_STACK' | 'SDXL_TUPLE' | 'BOOLEAN';
...
export const NodeRegistry: Record<string, NodeDefinition> = {
  ...
  // ── LTX-Video 2.x family (ComfyUI-LTXVideo + KJNodes helpers) ──────────
  // Real workflow shape (ltx-2.3-22b): the sampler chain runs
  // SamplerCustomAdvanced → CFGGuider → LTXVConditioning → CLIPTextEncode,
  // with sigmas from LTXVScheduler (carries `steps`). The prompt text is
  // generated by TextGenerateLTX2Prompt and LINKED into CLIPTextEncode's
  // text input — without registrations here, standard traversal dies at the
  // first unknown node and the global fallback loses steps/scheduler.
  //
  // For every node below, `inputs` ORDER mirrors the UI node's slot order
  // (createNodeMap names nameless UI links via Object.keys(inputs)[slot]).
  TextGenerateLTX2Prompt: {
    category: 'IO', roles: ['SOURCE'],
    // slots 0/1 are `clip` and `image` (real UI order); `prompt` is the
    // LLM-generator widget whose scalar value arrives via the execution
    // prompt. A prompt-first order misnames the CLIP link and clobbers it.
    inputs: {
      clip: { type: 'CLIP' }, image: { type: 'IMAGE' },
      prompt: { type: 'STRING' }, max_length: { type: 'INT' },
      sampling_mode: { type: 'STRING' },
      'sampling_mode.temperature': { type: 'FLOAT' },
      'sampling_mode.top_k': { type: 'INT' },
      'sampling_mode.top_p': { type: 'FLOAT' },
      'sampling_mode.min_p': { type: 'FLOAT' },
      'sampling_mode.repetition_penalty': { type: 'FLOAT' },
      'sampling_mode.seed': { type: 'INT' },
    },
    outputs: { text: { type: 'STRING' } },
    param_mapping: {
      prompt: { source: 'input', key: 'prompt' },
    },
    widget_order: ['prompt', 'max_length', 'sampling_mode']
  },

  LTXVScheduler: {
    category: 'UTILS', roles: ['TRANSFORM'],
    // slot 0 is `latent` (LATENT), not a model input. Real widgets are
    // [steps, max_shift, base_shift, stretch, terminal] — LTX-Video 2.x
    // has NO named scheduler (the sigma schedule is fixed by the shift
    // params), so `scheduler` intentionally stays null for this family.
    inputs: { latent: { type: 'LATENT' } },
    outputs: { SIGMAS: { type: 'SIGMAS' } },
    param_mapping: {
      steps: { source: 'widget', key: 'steps' },
    },
    widget_order: ['steps', 'max_shift', 'base_shift', 'stretch', 'terminal'],
    pass_through_rules: []
  },

  LTXVConditioning: {
    category: 'CONDITIONING', roles: ['TRANSFORM'],
    // IMPORTANT: input ORDER mirrors the node's slot order (positive=0,
    // negative=1, frame_rate=2, model=3). UI links without explicit input
    // names are mapped by slot index (Object.keys(inputs)[slot]) — a
    // misaligned order would overwrite the execution prompt's values and
    // the negative encode could be picked up as the prompt.
    inputs: {
  ...
  LTXVCropGuides: {
    category: 'CONDITIONING', roles: ['TRANSFORM'],
    // Real slot order: positive, negative, latent. With a `conditioning`
    // input instead, the upscale-sampler chain (SCA → CFGGuider →
    // LTXVCropGuides → LTXVConditioning) can't traverse the conditioning.
    inputs: {
      positive: { type: 'CONDITIONING' },
      negative: { type: 'CONDITIONING' },
      latent: { type: 'LATENT' },
  ...
  LTXVEmptyLatentAudio: {
    category: 'SAMPLING', roles: ['SOURCE'],
    // Real slot order: audio_vae, frames_number, frame_rate.
    inputs: {
      audio_vae: { type: 'VAE' }, frames_number: { type: 'INT' }, frame_rate: { type: 'INT' },
      batch_size: { type: 'INT' },
    },
    outputs: { LATENT: { type: 'LATENT' } },
    param_mapping: {},
    widget_order: ['batch_size']
  },

  EmptyLTXVLatentVideo: {
    category: 'SAMPLING', roles: ['SOURCE'],
    // Real slot order: width, height, length (batch_size is a widget).
    inputs: { width: { type: 'INT' }, height: { type: 'INT' }, length: { type: 'INT' }, batch_size: { type: 'INT' } },
    outputs: { LATENT: { type: 'LATENT' } },
    param_mapping: {},
  ...
  LTXVConcatAVLatent: {
    category: 'SAMPLING', roles: ['TRANSFORM'],
    inputs: { video_latent: { type: 'LATENT' }, audio_latent: { type: 'LATENT' } },
    outputs: { LATENT: { type: 'LATENT' } },
    param_mapping: {},
  },
  ...
  LTXVSeparateAVLatent: {
    category: 'SAMPLING', roles: ['TRANSFORM'],
    inputs: { latent: { type: 'LATENT' } },
    outputs: { video: { type: 'LATENT' }, audio: { type: 'LATENT' } },
    param_mapping: {},
  },

  LTXVLatentUpsampler: {
    category: 'SAMPLING', roles: ['TRANSFORM'],
    // Real slot order: samples, upscale_model, vae.
    inputs: {
      samples: { type: 'LATENT' }, upscale_model: { type: 'LATENT_UPSCALE_MODEL' }, vae: { type: 'VAE' },
    },
    outputs: { LATENT: { type: 'LATENT' } },
    param_mapping: {},
  },
  ...
  LTXVAudioVAEDecode: {
    category: 'IO', roles: ['TRANSFORM'],
    // Real slot order: samples, audio_vae.
    inputs: { samples: { type: 'LATENT' }, audio_vae: { type: 'VAE' } },
  ...
  CreateVideo: {
    category: 'IO', roles: ['TRANSFORM'],
    // Real slot order: images, audio, fps.
    inputs: { images: { type: 'IMAGE' }, audio: { type: 'AUDIO' }, fps: { type: 'FLOAT' } },
    outputs: { IMAGE: { type: 'IMAGE' } },
    param_mapping: {},
  },
  ...
  SaveVideo: {
    category: 'IO', roles: ['SINK'],
    inputs: { video: { type: 'VIDEO' }, filename_prefix: { type: 'STRING' }, format: { type: 'STRING' }, codec: { type: 'STRING' } },
    outputs: { IMAGE: { type: 'IMAGE' } },
    param_mapping: {},
    widget_order: ['filename_prefix', 'format', 'codec']
  },

  EmptyImage: {
    category: 'IO', roles: ['SOURCE'],
    inputs: { width: { type: 'INT' }, height: { type: 'INT' }, batch_size: { type: 'INT' }, color: { type: 'INT' } },
    outputs: { IMAGE: { type: 'IMAGE' } },
    param_mapping: {},
    widget_order: ['width', 'height', 'batch_size', 'color']
  },

  GetImageSize: {
    category: 'UTILS', roles: ['TRANSFORM'],
    inputs: { image: { type: 'IMAGE' } },
    outputs: { width: { type: 'INT' }, height: { type: 'INT' }, batch_size: { type: 'INT' } },
    param_mapping: {},
  },
  ...
  ManualSigmas: {
    category: 'UTILS', roles: ['SOURCE'],
    inputs: { sigmas: { type: 'STRING' }, interpolate: { type: 'BOOLEAN' } },
    outputs: { SIGMAS: { type: 'SIGMAS' } },
    param_mapping: {},
    widget_order: ['sigmas', 'interpolate']
  },

  VAELoaderKJ: {
    category: 'LOADING', roles: ['SOURCE'],
    inputs: { vae_name: { type: 'STRING' }, device: { type: 'STRING' }, weight_dtype: { type: 'STRING' } },
    outputs: { VAE: { type: 'VAE' } },
    param_mapping: {
      vae: { source: 'widget', key: 'vae_name' }
    },
    widget_order: ['vae_name', 'device', 'weight_dtype']
  },

  LatentUpscaleModelLoader: {
    category: 'LOADING', roles: ['SOURCE'],
    inputs: { model_name: { type: 'STRING' } },
    outputs: { UPSCALE_MODEL: { type: 'LATENT_UPSCALE_MODEL' } },
    param_mapping: {},
  ...
  SamplerCustomAdvanced: {
    ...
    param_mapping: {
      ...
      prompt: { source: 'trace', input: 'guider' },
      negativePrompt: { source: 'trace', input: 'guider' },
